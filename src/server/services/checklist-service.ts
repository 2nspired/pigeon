...
import type { CreateChecklistItemInput, UpdateChecklistItemInput } from "@/lib/schemas/checklist-schemas";
...
async function create(data: CreateChecklistItemInput): Promise<ServiceResult<ChecklistItem>> {
	try {
		...
		return { success: false, error: { code: "CREATE_FAILED", message: "Failed to create checklist item." } };
	}
}
...
async function update(
	...
	try {
		...
		return { success: false, error: { code: "UPDATE_FAILED", message: "Failed to update checklist item." } };
	}
}
...
async function deleteItem(itemId: string): Promise<ServiceResult<ChecklistItem>> {
	try {
		...
		return { success: false, error: { code: "DELETE_FAILED", message: "Failed to delete checklist item." } };
	}
}

	...
	cards: [
		...
		},
		{
			...
			description: [
				...
				'**Try it (agent):** `planCard({ boardId, cardId: "#6" })` — an essential tool, directly callable. Read the returned `protocol`, walk the four steps (investigate → synthesize → propose in chat → publish on confirm). Watch the activity strip — a `planning` event is stamped while the plan is in flight.',

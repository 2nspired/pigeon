---
title: Concepts
description: How Pigeon works — the problem, the two readers, sessions, handoffs, the briefMe loop, the saveHandoff wrap-up, and the surfaces. The why under the why.
---
...
| **Your agent** | Structured tools over MCP. Cards, columns, comments, handoffs, decisions, dependencies, code links. | 11 essential tools always-on; 65+ extended tools discoverable via `getTools` and called via `runTool`. Context footprint stays small. |
...
The rule: *if the human can't see and edit it where they'd naturally encounter it, the agent shouldn't trust it.* Full guide: [Write a tracker.md](/tracker-md/).
...
Pigeon's MCP server registers under the key `pigeon` and runs over stdio. Eleven essential tools are always-on (`briefMe`, `saveHandoff`, `createCard`, `updateCard`, `moveCard`, `addComment`, `planCard`, `registerRepo`, `checkOnboarding`, `getTools`, `runTool`). The remaining 65+ tools — cards, checklist, context, decisions, diagnostics, digest, discovery, git, knowledge, milestones, notes, relations, sessions, setup, tags — are discoverable via `getTools` and executable via `runTool`. The full reference, with parameter schemas: [MCP tools](/tools/).
...
| **Permanent** | The `tracker.db` filename, the `tracker.md` filename, the stable MCP tool names (`briefMe`, `saveHandoff`, `createCard`, `updateCard`, `moveCard`, `addComment`, `getTools`, `runTool`), Prisma table names, and the `tracker://` URI scheme. None of these change. |

`npm run doctor` runs eight checks against your install and tells you exactly what's drifted. Run it after install, after every `git pull`, and after `npm run service:update`. Exit code is `0` on green, `1` on any failure — CI-friendly. Full semver + schema-version policy lives in [`docs/VERSIONING.md`](https://github.com/2nspired/pigeon/blob/main/docs/VERSIONING.md).

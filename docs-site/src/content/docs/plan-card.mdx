...
2. **In Claude Code, run the slash command:**
   ...
   In any other agent (Codex, Cursor, Windsurf), call the tool directly — `planCard` is an essential tool, no `runTool` wrapper needed:

   ```
   planCard({ boardId: '<board uuid>', cardId: '#42' })
   ```

...
- **11 essential tools** are always loaded. They cover the hot path: start a session, create a card, plan it, update it, move it, comment on it, save a handoff at the end.
- **65+ extended tools** live behind two meta-tools: `getTools` browses them by category, `runTool` executes them by name.
...
- **You can see the handoff.** It's a row in the `Handoff` table. You can read it, edit it, delete it.
